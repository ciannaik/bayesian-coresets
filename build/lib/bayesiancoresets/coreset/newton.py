...
class ApproxNewtonCoreset(Coreset):
  def __init__(self, data, ll_projector, n_subsample_select=None, n_subsample_opt=None, opt_itrs=100, step_sched=lambda i : 1./(i+1), **kw):
    self.data = data
    self.cts = []
    self.ct_idcs = []
  ...
  def _optimize(self):
    def grd(w,alpha=0.00001):
      # print("w = {}".format(w))
      # vecs_sum, sum_scaling, sub_idcs, corevecs = self._get_projection(self.n_subsample_opt, w, self.pts,return_sum=True)
      vecs, sum_scaling, sub_idcs, corevecs = self._get_projection(self.n_subsample_opt, w, self.pts,return_sum=False)
      ...
      # print("corevecs_cov = {}".format(corevecs_cov))
      # print(np.linalg.det(corevecs_cov))
      # # corevecs_cov_inv_old = np.linalg.solve(corevecs.dot(corevecs.T)/corevecs.shape[1])
      #
      # Lcorevecs_cov = np.linalg.cholesky(corevecs_cov)
      # Lcorevecs_cov_inv = solve_triangular(Lcorevecs_cov, np.eye(Lcorevecs_cov.shape[0]), lower=True, overwrite_b=True, check_finite=False)
      # corevecs_cov_inv = np.triu(Lcorevecs_cov_inv.T,1) + Lcorevecs_cov_inv
      ...
      # print("grd_unscaled = {}".format(grd_unscaled))

      grd = np.linalg.solve(corevecs_cov,grd_unscaled)
      # print("grd = {}".format(grd))
      return grd
    x0 = self.wts
    self.wts = an_opt(x0, grd, opt_itrs = self.opt_itrs, step_sched = self.step_sched)
